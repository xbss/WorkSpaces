...
public class AdminGoodEdit extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (!isLOGINNAME && !isPASSWORD && !isSEX && !isBIRTH && !isPHONE
			...
			// 没有错就跳转到注册成功界面清除session全部信息并以新用户登陆
			...
			user.setSEX(SEX);
			user.setBIRTH(new java.sql.Date(new Date(BIRTH).getTime())
					.toString());
			user.setPHONE(PHONE);
			user.setEMAIL(MAIL);
			...
			// 如果有错的话及回到注册页面
			req.getRequestDispatcher("modules/admin/jsp/AdminUserEdit.jsp")
					.forward(req, resp);
